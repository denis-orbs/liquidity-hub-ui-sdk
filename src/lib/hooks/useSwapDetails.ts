...
export function usePriceImpact() {
  ...
  return useMemo(() => {
    return BN(outTokenUsdAmount || "0")
      .div(inTokenUsdAmount || "0")
      .minus(1)
      .times(100)
      .toString();
...
export function useGasCostUsd() {
    ...
    return useMemo(() => {
      ...
      return amountUi(toToken?.decimals, BN(gasCostOutputToken).multipliedBy(outTokenUsd));
  ...
  export const useSwapConfirmation = () => {
    ...
    const outAmount = useAmountUI(store.toToken?.decimals, useQuote().data?.outAmount);
